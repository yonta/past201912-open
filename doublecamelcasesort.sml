fun parseImpl results NONE nil = rev results
  | parseImpl _ (SOME _) nil = raise Fail "bug: not coupled upcase"
  | parseImpl results NONE (c :: cs) =
    if Char.isUpper c then parseImpl results (SOME [c]) cs
    else raise Fail "bug: not started upcase"
  | parseImpl results (SOME chars) (c :: cs) =
    if Char.isUpper c
    then let val word = String.implode (rev (c :: chars))
         in  parseImpl (word :: results) NONE cs end
    else parseImpl results (SOME (c :: chars)) cs
fun parse line = parseImpl nil NONE (String.explode line)
fun toLowers words = map (String.map Char.toLower) words
fun toHeadUpper nil = nil
  | toHeadUpper (c :: cs) = Char.toUpper c :: cs
fun toTailUpper nil = nil
  | toTailUpper [c] = [Char.toUpper c]
  | toTailUpper (c :: cs) = c :: toTailUpper cs
fun toDoubleUppers words =
    let
      val css = map String.explode words
      val css = map (fn cs => toTailUpper (toHeadUpper cs)) css
    in
      map String.implode css
    end
...
fun sort words =
    let
      val arr = Array.fromList words
      val () = ArrayQSort.sort String.compare arr
    in
      arrayToList arr
    end
...
fun main () =
    let
      ...
      val stringReader = String.scan getc
      val (inputLine, _) = valOf (stringReader instream)
                           handle Option => raise Fail "bug: invalid input"
      val inputs = parse inputLine
      val lowers = toLowers inputs
      val sorted  = sort lowers
      val answer = toDoubleUppers sorted
    in
      app print answer
    end
val () = main ()
